...
class ModelNetDataset():
    def __init__(self, root, batch_size = 32, npoints = 1024, split='train', normalize=True, 
            normal_channel=False, modelnet10=False, cache_size=15000, shuffle=None, rotate=False):
        self.root = root
    ...
    def _augment_batch_data(self, batch_data):
        if self.normal_channel:
            ...
            rotated_data = provider.rotate_perturbation_point_cloud_with_normal(batch_data)

        else:
            if self.rotate:
                batch_data = provider.rotate_point_cloud(batch_data)
            rotated_data = provider.rotate_perturbation_point_cloud(batch_data)
