...
parser.add_argument('--model', default='pan3_pseg', help='Model name [default: model]')
parser.add_argument('--log', default='log_pseg', help='Log dir [default: log_pseg]')
parser.add_argument('--num_point', type=int, default=2048, help='Point Number [default: 2048]')
parser.add_argument('--max_epoch', type=int, default=201, help='Epoch to run [default: 201]')
parser.add_argument('--batch_size', type=int, default=32, help='Batch Size during training [default: 32]') # 16
...
parser.add_argument('--cls_loss', type=float, default=0.1, help='Weights for classification loss') # 0
parser.add_argument('--coarse_loss', type=float, default=1, help='Weights for coarse loss')
parser.add_argument('--mmd_loss', type=float, default=200, help='Weights for mmd loss')
parser.add_argument('--no_pfs', action='store_true', help='Whether to use pfs')
...
BN_DECAY_CLIP = 0.99 # (default 0.99)

LR_DECAY_CLIP = 1e-5 # (default 1e-5)
...
def get_learning_rate(batch):
	learning_rate = tf.train.exponential_decay(
						...
						staircase=True)
	learning_rate = tf.maximum(learning_rate, LR_DECAY_CLIP) # CLIP THE LEARNING RATE! 
...
def train():
	with tf.Graph().as_default():
		with tf.device('/gpu:'+str(GPU_INDEX)):
			...
			# Get model and loss 
			pred, end_points = MODEL.get_model(pointclouds_pl, labels_pl, cls_labels_pl, is_training_pl, bn_decay, 
				CLS_FLAG, COARSE_FLAG, MMD_FLAG, PFS_FLAG, FUCT_FLAG)
			loss = MODEL.get_loss(pred, labels_pl, cls_labels_pl, end_points, COARSE_FLAG, CLS_FLAG, MMD_FLAG)
		...
		for epoch in range(MAX_EPOCH):
			...
			best_mean_iou_flag, best_all_mean_iou_flag = eval_one_epoch(sess, ops, test_writer)

			if best_mean_iou_flag:
				save_path = saver.save(sess, os.path.join(LOG_DIR, "best_mean_iou_model.ckpt"))
				log_string("Model saved in file: %s" % save_path)

			if best_all_mean_iou_flag:
				save_path = saver.save(sess, os.path.join(LOG_DIR, "best_all_mean_iou_model.ckpt"))
				log_string("Model saved in file: %s" % save_path)
...
def eval_one_epoch(sess, ops, test_writer):
	...
	global BEST_ALL_MEAN_IOU

	is_training = False
	test_idxs = np.arange(0, len(TEST_DATASET))
	# Test on all data: last batch might be smaller than BATCH_SIZE
	num_batches = int((np.floor(len(TEST_DATASET)+BATCH_SIZE-1)/BATCH_SIZE))
	...
	for batch_idx in range(num_batches):
		...
		if CLS_FLAG > 0:
			feed_dict = {ops['pointclouds_pl']: batch_data,
						 ...
						 ops['cls_labels_pl']: batch_cls_label,
						 ops['is_training_pl']: is_training}
			summary, step, loss_val, pred_val, cls_pred_val = sess.run([ops['merged'], ops['step'],
				ops['loss'], ops['pred'], ops['cls_pred']], feed_dict=feed_dict)
		else:
			feed_dict = {ops['pointclouds_pl']: batch_data,
						 ops['labels_pl']: batch_label,
						 ops['cls_labels_pl']: batch_cls_label,
		...
		if CLS_FLAG > 0:
			cur_cls_pred_val = cls_pred_val[0:cur_batch_size]
			
			cur_cls_pred_val = np.argmax(cur_cls_pred_val, 1)
			cls_correct = np.sum(cur_cls_pred_val == batch_cls_label)
			total_cls_correct += cls_correct
			total_cls_seen += cur_batch_size
	...
	all_mean_shape_ious = np.mean(all_shape_ious)
	log_string('eval mean loss: %f' % (loss_sum / float(len(TEST_DATASET)/BATCH_SIZE)))
	
	if CLS_FLAG > 0:
		log_string('eval cls accuracy: %f' % (total_cls_correct / float(total_cls_seen)))
	...
	log_string('eval mean mIoU (all shapes): %f' % (all_mean_shape_ious))

	best_mean_iou_flag, best_all_mean_iou_flag = False, False
	if mean_shape_ious > BEST_MEAN_IOU:
		BEST_MEAN_IOU = mean_shape_ious
		best_mean_iou_flag = True
	if all_mean_shape_ious > BEST_ALL_MEAN_IOU:
		BEST_ALL_MEAN_IOU = all_mean_shape_ious
		best_all_mean_iou_flag = True

	log_string('best eval mean mIoU: %f' % (BEST_MEAN_IOU))
	log_string('best eval all mean mIoU: %f' % (BEST_ALL_MEAN_IOU))
		 
	EPOCH_CNT += 1
	# return total_correct/float(total_seen)
	return (best_mean_iou_flag, best_all_mean_iou_flag)
