...
REGISTER_OP("PrincipalFeatureSample")
  .Attr("npoint: int")
  .Input("inf: float32")
  .Input("starting_index: int32")
  .Output("out: int32")
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ::tensorflow::shape_inference::ShapeHandle dims1; // batch_size * ndataset * ndataset
    c->WithRank(c->input(0), 3, &dims1);
    ::tensorflow::shape_inference::ShapeHandle dims2; // batch_size
    c->WithRank(c->input(0), 1, &dims2);
    int npoint;
    TF_RETURN_IF_ERROR(c->GetAttr("npoint", &npoint));
    ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims1, 0), npoint});
    c->set_output(0, output);
...
void principalfeaturesamplingLauncher(int b, int n, int m, const float * inf, const int * starting_index, float * temp, int * out);
class PrincipalFeatureSampleGpuOp: public OpKernel{
  public:
    explicit PrincipalFeatureSampleGpuOp(OpKernelConstruction* context):OpKernel(context) {
                  OP_REQUIRES_OK(context, context->GetAttr("npoint", &npoint_));
                  OP_REQUIRES(context, npoint_ > 0, errors::InvalidArgument("PrincipalFeatureSample expects positive npoint"));
              }
    void Compute(OpKernelContext * context)override{
      int m = npoint_;

      const Tensor& inf_tensor=context->input(0);
      OP_REQUIRES(context, inf_tensor.dims()==3, errors::InvalidArgument("PrincipalFeatureSample expects (batch_size, ndataset, ndataset) inf shape"));
      int b=inf_tensor.shape().dim_size(0);
      int n=inf_tensor.shape().dim_size(1);
      auto inf_flat=inf_tensor.flat<float>();
      const float * inf=&(inf_flat(0));

      const Tensor& starting_index_tensor=context->input(1);
      OP_REQUIRES(context, starting_index_tensor.dims()==1 && starting_index_tensor.shape().dim_size(0)==b, errors::InvalidArgument("PrincipalFeatureSample expects (batch_size) starting_index shape"));
      auto starting_index_flat=starting_index_tensor.flat<int>();
      const int * starting_index=&(starting_index_flat(0));
      ...
      auto temp_flat=temp_tensor.flat<float>();
      float * temp=&(temp_flat(0));
      principalfeaturesamplingLauncher(b,n,m,inf,starting_index, temp, out);
    }
    private:
        int npoint_;
};
REGISTER_KERNEL_BUILDER(Name("PrincipalFeatureSample").Device(DEVICE_GPU),PrincipalFeatureSampleGpuOp);
