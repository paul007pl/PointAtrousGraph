...
__global__ void principalfeaturesamplingKernel(int b, int n, int m, const float * __restrict__ dataset, 
	const int * __restrict__ starting_index, float * __restrict__ temp, int * __restrict__ idxs){
	if (m<=0)
		return;
	const int BlockSize=512;
	__shared__ float dists[BlockSize];
	...
	for (int i = blockIdx.x; i < b; i+=gridDim.x){
		...
		for (int j = 1; j < m; j ++){
			int besti = starting_index[i];
			float best=-1;
			for(int k = threadIdx.x; k < n; k += blockDim.x){
				float td=temp[blockIdx.x*n+k];
				float d = dataset[i*n*n+old*n+k];
				float d2 = min(d,td);
				if (d2!=td)
					temp[blockIdx.x*n+k]=d2;
				if (d2>best){
...
__global__ void gatherpointKernel(int b,int n,int m,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out){
	for (int i=blockIdx.x;i<b;i+=gridDim.x){ // batch_size
		for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
			int a=idx[i*m+j];
...
void principalfeaturesamplingLauncher(int b, int n, int m, const float * inf, const int * starting_index, float * temp, int * out){
	principalfeaturesamplingKernel<<<32,512>>>(b,n,m,inf,starting_index, temp, out);
}
